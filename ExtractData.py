...
def GetIUtahData():
    ...
    data_array_iUtah = [[defaultValue] * 25 for row in range(0, num_sites+1)]
    ...
    for siteCode_loganriverIndex, siteCode_loganriverShort in enumerate(siteCodes):
        ...
        data_array_iUtah[i][20] = defaultValue if value_ST5cm == -9999.0 else round(value_ST5cm, 2)  # ST_5cm
        ...
        data_array_iUtah[i][21] = defaultValue if value_ST10cm == -9999.0 else round(value_ST10cm, 2)  # ST_10cm
        ...
        data_array_iUtah[i][22] = defaultValue if value_ST20cm == -9999.0 else round(value_ST20cm, 2)  # ST_20cm
        ...
        data_array_iUtah[i][23] = defaultValue if value_ST50cm == -9999.0 else round(value_ST50cm, 2)  # ST_50cm
        ...
        data_array_iUtah[i][24] = defaultValue if value_ST100cm == -9999.0 else round(value_ST100cm, 2)  # ST_100cm
        '''print (
...
def GetScanData(count):
    ...
    # * get stations' info from SCAN *#
    stations = awdb.service.getStations("*", state, network, "*", "*", -1000, 1000, -1000, 1000, 0, 29000, "*", 1, None, True)  # get stations' info from SCAN
...
def GetMesoWestData(count):
    ...
    for validSite in validSite_sms:
        ...
        for j in range(0, NumberOfDays):
            dayPrecipitate = r.precip(state='UT', stid=station_id, start=PrecipitationPeriod[j + 1].beginDate_Meso, end=starDate_Meso_str,
                                       units='precip|in', showemptystations=1)
