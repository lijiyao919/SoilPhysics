...
class Precipitation(object):
    ...
    # * retrieving date *#
startDate = dt.strptime("2017-09-23 00:00", "%Y-%m-%d %H:%M")  # date to retrieve soil moisture and precipitation
...
def GetIUtahData(count):
    ...
    for siteCode_loganriverIndex, siteCode_loganriverShort in enumerate(siteCodes):
        ...
        print (
            data_array_iUtah[i][0],data_array_iUtah[i][1],data_array_iUtah[i][2],data_array_iUtah[i][3],data_array_iUtah[i][4],
            ...
            data_array_iUtah[i][17], data_array_iUtah[i][18], data_array_iUtah[i][19], data_array_iUtah[i][20], data_array_iUtah[i][21],
...
def GetSnortelData(count):
    ...
    if meta:
        i = 0  # iteration for valid stations
            # validSite1=filter(lambda x:x.stationTriplet=="1113:UT:SNTL",meta)# code to check for particular station
        ...
        for validSite in meta:
            ...
            sm_2 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_SMS, 1, heights_2, starDate_Snortel_str,
                                                     starDate_Snortel_str, 'ALL', 'ENGLISH')  # soil moisture (pct) -2 inch
            ...
            sm_4 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_SMS, 1, heights_4, starDate_Snortel_str,
                                                     starDate_Snortel_str, 'ALL', 'ENGLISH')  # soil moisture (pct) -4 inch
            ...
            data_array_snortel[i][18] = defaultValue if 'beginDate' not in sm_20[0] or 'endDate' not in sm_20[0] or \
                                                sm_20[0].values[0] == "" or 'value' not in sm_20[0].values[0] \
            ...
            data_array_snortel[i][19] = defaultValue if 'beginDate' not in sm_40[0] or 'endDate' not in sm_40[0] or \
                                                sm_40[0].values[0] == "" or 'value' not in sm_40[0].values[0] \
            ...
            data_array_snortel[i][23] = defaultValue if 'beginDate' not in st_20[0] or 'endDate' not in st_20[0] or \
                                                st_20[0].values[0] == "" or 'value' not in st_20[0].values[0] \
            ...
            data_array_snortel[i][24] = defaultValue if 'beginDate' not in st_40[0] or 'endDate' not in st_40[0] or \
                                                st_40[0].values[0] == "" or 'value' not in st_40[0].values[0] \
            ...
            print (
                data_array_snortel[i][0], data_array_snortel[i][1], data_array_snortel[i][2], data_array_snortel[i][3], data_array_snortel[i][4], data_array_snortel[i][5],
                ...
                data_array_snortel[i][13], data_array_snortel[i][14], data_array_snortel[i][15], data_array_snortel[i][16], data_array_snortel[i][17],
                data_array_snortel[i][18],
                data_array_snortel[i][19], data_array_snortel[i][20], data_array_snortel[i][21], data_array_snortel[i][22], data_array_snortel[i][23],data_array_snortel[i][24])
            i += 1
...
def GetScanData(count):
    ...
    if meta:
        ...
        for validSite in meta:
            ...
            sm_2 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_SMS, 1, heights_2, starDate_Snortel_str,
                                                     starDate_Snortel_str, 'ALL', 'ENGLISH')  # soil moisture (pct) -2 inch
            data_array_scan[i][15] = -999999 if 'beginDate' not in sm_2[0] or 'endDate' not in sm_2[0] or sm_2[0].values[
                                                                                                         0] == "" or 'value' not in \
                                                                                                                     sm_2[
                                                                                                                         0].values[
                                                                                                                         0] \
            ...
            sm_4 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_SMS, 1, heights_4, starDate_Snortel_str,
                                                     starDate_Snortel_str, 'ALL', 'ENGLISH')  # soil moisture (pct) -4 inch
            data_array_scan[i][16] = -999999 if 'beginDate' not in sm_4[0] or 'endDate' not in sm_4[0] or sm_4[0].values[
                                                                                                         0] == "" or 'value' not in \
                                                                                                                     sm_4[
                                                                                                                         0].values[
                                                                                                                         0] \
            ...
            sm_8 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_SMS, 1, heights_8,
                                                     starDate_Snortel_str, starDate_Snortel_str, 'ALL',
                                                     'ENGLISH')  # soil moisture (pct) -8 inch
            data_array_scan[i][17] = -999999 if 'beginDate' not in sm_8[0] or 'endDate' not in sm_8[0] or sm_8[0].values[
                                                                                                         0] == "" or 'value' not in \
                                                                                                                     sm_8[
                                                                                                                         0].values[
                                                                                                                         0] \
            ...
            sm_20 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_SMS, 1, heights_20,
                                                      starDate_Snortel_str, starDate_Snortel_str, 'ALL',
                                                      'ENGLISH')  # soil moisture (pct) -20 inch
            data_array_scan[i][18] = -999999 if 'beginDate' not in sm_20[0] or 'endDate' not in sm_20[0] or sm_20[0].values[
                                                                                                           0] == "" or 'value' not in \
                                                                                                                       sm_20[
                                                                                                                           0].values[
                                                                                                                           0] \
            ...
            sm_40 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_SMS, 1, heights_40,
                                                      starDate_Snortel_str, starDate_Snortel_str, 'ALL',
                                                      'ENGLISH')  # soil moisture (pct) -40 inch
            data_array_scan[i][19] = -999999 if 'beginDate' not in sm_40[0] or 'endDate' not in sm_40[0] or sm_40[0].values[
                                                                                                           0] == "" or 'value' not in \
                                                                                                                       sm_40[
                                                                                                                           0].values[
                                                                                                                           0] \
            ...
            st_2 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_STO, 1, heights_2,
                                                     starDate_Snortel_str, starDate_Snortel_str, 'ALL',
                                                     'ENGLISH')  # soil temperature (Fahrenheit) -2 inch
            data_array_scan[i][20] = -999999 if 'beginDate' not in st_2[0] or 'endDate' not in st_2[0] or st_2[0].values[
                                                                                                         0] == "" or 'value' not in \
                                                                                                                     st_2[
                                                                                                                         0].values[
                                                                                                                         0] \
            ...
            st_4 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_STO, 1, heights_4,
                                                     starDate_Snortel_str, starDate_Snortel_str, 'ALL',
                                                     'ENGLISH')  # soil temperature (Fahrenheit) -4 inch
            data_array_scan[i][21] = -999999 if 'beginDate' not in st_4[0] or 'endDate' not in st_4[0] or st_4[0].values[
                                                                                                         0] == "" or 'value' not in \
                                                                                                                     st_4[
                                                                                                                         0].values[
                                                                                                                         0] \
            ...
            st_8 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_STO, 1, heights_8,
                                                     starDate_Snortel_str, starDate_Snortel_str, 'ALL',
                                                     'ENGLISH')  # soil temperature (Fahrenheit) -8 inch
            data_array_scan[i][22] = -999999 if 'beginDate' not in st_8[0] or 'endDate' not in st_8[0] or st_8[0].values[
                                                                                                         0] == "" or 'value' not in \
                                                                                                                     st_8[
                                                                                                                         0].values[
                                                                                                                         0] \
            ...
            st_20 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_STO, 1, heights_20,
                                                      starDate_Snortel_str, starDate_Snortel_str, 'ALL',
                                                      'ENGLISH')  # soil temperature (Fahrenheit) -20 inch
            data_array_scan[i][23] = -999999 if 'beginDate' not in st_20[0] or 'endDate' not in st_20[0] or st_20[0].values[
                                                                                                           0] == "" or 'value' not in \
                                                                                                                       st_20[
                                                                                                                           0].values[
                                                                                                                           0] \
            ...
            st_40 = awdb.service.getInstantaneousData(validSite.stationTriplet, sensor_STO, 1, heights_40,
                                                      starDate_Snortel_str, starDate_Snortel_str, 'ALL',
                                                      'ENGLISH')  # soil temperature (Fahrenheit) -40 inch
            data_array_scan[i][24] = -999999 if 'beginDate' not in st_40[0] or 'endDate' not in st_40[0] or st_40[0].values[
                                                                                                           0] == "" or 'value' not in \
                                                                                                                       st_40[
                                                                                                                           0].values[
                                                                                                                           0] \
            ...
            for j in range(0, NumberOfDays):
                ...
                data_array_scan[i][10 + j] = None if 'beginDate' not in dayPrecipitate[0] or 'endDate' not in dayPrecipitate[
                    0] or 'beginDate' not in prec[0] or 'endDate' not in prec[0] \
                                               or prec[0].values[0].value < dayPrecipitate[0].values[0].value \
                    else 25.4 * (prec[0].values[0].value - dayPrecipitate[0].values[0].value)

            print (
                data_array_scan[i][0], data_array_scan[i][1], data_array_scan[i][2], data_array_scan[i][3], data_array_scan[i][4], data_array_scan[i][5],
                data_array_scan[i][6],
                data_array_scan[i][7], data_array_scan[i][8], data_array_scan[i][9], data_array_scan[i][10], data_array_scan[i][11],
                data_array_scan[i][12],
                data_array_scan[i][13], data_array_scan[i][14], data_array_scan[i][15], data_array_scan[i][16], data_array_scan[i][17],
                data_array_scan[i][18],
                data_array_scan[i][19], data_array_scan[i][20], data_array_scan[i][21], data_array_scan[i][22], data_array_scan[i][23])
            i += 1
...
def GetMesoWestData(count):
    ...
    for validSite in validSite_sms:
        ...
        if num_sms_positions == 1 and num_sto_positions == 1:
            print (data_array_Meso[i][0], data_array_Meso[i][1], data_array_Meso[i][2], data_array_Meso[i][3], data_array_Meso[i][4],
                   data_array_Meso[i][5], data_array_Meso[i][6], data_array_Meso[i][7], data_array_Meso[i][8], data_array_Meso[i][9],
...
try:
    ...
    if countError==0:
        try:
            ...
            raise Exception("Iutah server is not responding")
SnortelArray=GetSnortelData(len(IUtahArray))# Data from Snortel Network
ScanArray=GetScanData(len(IUtahArray)+len(SnortelArray)) #Data from Scan Network
MesoWestArray=GetMesoWestData(len(IUtahArray)+len(SnortelArray)+len(ScanArray))#Data from MesoWest Network
data_array = np.vstack((IUtahArray, SnortelArray,ScanArray,MesoWestArray)) #Combining data from all networks
